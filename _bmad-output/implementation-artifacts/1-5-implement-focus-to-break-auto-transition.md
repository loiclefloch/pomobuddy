...
- [ ] Task 1: Update Timer Zustand Store for Break State (AC: #1, #2)
  - [ ] 1.1: Update `useTimerStore` to handle 'break' status correctly
  - [ ] 1.2: Add break-specific derived state (isBreak, isActive helpers)
  - [ ] 1.3: Verify store handles transitions: focus->break->idle seamlessly

- [ ] Task 2: Update TimerDisplay for Break Visual Indicator (AC: #2)
  - [ ] 2.1: Add break state styling using `cozy-success` (#A8C5A0) color
  - [ ] 2.2: Display "Break" label when timer is in break mode
  - [ ] 2.3: Use different visual indicator (icon or text) for break vs focus
  - [ ] 2.4: Update `TimerDisplay.test.tsx` for break state rendering

- [ ] Task 3: Update TimerControls for Break Mode (AC: #3)
  - [ ] 3.1: Ensure Pause/Resume buttons work during break mode
  - [ ] 3.2: Ensure Stop button is available during break mode
  - [ ] 3.3: Stop during break returns to idle, not to focus
  - [ ] 3.4: Update `TimerControls.test.tsx` for break state behavior

- [ ] Task 4: Handle SessionComplete Event for Auto-Transition (AC: #1, #5)
  - [ ] 4.1: Update `useTimer` hook to listen for `SessionComplete` event
  - [ ] 4.2: On `SessionComplete` with sessionType 'focus', UI should expect break to start
  - [ ] 4.3: Backend handles the actual transition (status change, remaining_seconds = 300)
  - [ ] 4.4: Frontend receives next `TimerTick` with break status and updates UI
  - [ ] 4.5: Add test for session complete handling

- [ ] Task 5: Handle Break Completion to Idle Transition (AC: #4)
  - [ ] 5.1: On `SessionComplete` with sessionType 'break', transition UI to idle
  - [ ] 5.2: Reset timer display to initial state
  - [ ] 5.3: Show Start button for next session
  - [ ] 5.4: Verify no auto-start of new focus session (user must click Start)

- [ ] Task 6: Visual Polish and Accessibility (AC: #1, #2)
  - [ ] 6.1: Add smooth transition animation between focus and break states
  - [ ] 6.2: Ensure color contrast meets WCAG AA for break state
  - [ ] 6.3: Update ARIA labels for break mode ("Break time remaining: X minutes")
  - [ ] 6.4: Respect `prefers-reduced-motion` for state transitions

- [ ] Task 7: Integration Testing (AC: #1-5)
  - [ ] 7.1: Test full cycle: Start -> 25min focus -> auto-break -> 5min break -> idle
  - [ ] 7.2: Test Stop during focus - returns to idle (no break)
  - [ ] 7.3: Test Stop during break - returns to idle immediately
  - [ ] 7.4: Verify UI color changes: coral (focus) -> sage (break) -> muted (idle)
  - [ ] 7.5: Verify timer displays correct countdown for both states
